package com.slilio.framework.common.response;

import java.util.List;
import lombok.Data;

/**
 * @Author: slilio @CreateTime: 2025-04-26 @Description: 分页响应工具类 @Version: 1.0
 */
@Data
public class PageResponse<T> extends Response<List<T>> {
  ...
  public static <T> PageResponse<T> success(
      List<T> data, long pageNo, long totalCount, long pageSize) {
    PageResponse<T> pageResponse = new PageResponse<>();
    pageResponse.setSuccess(true);
    pageResponse.setData(data);
    pageResponse.setPageNo(pageNo);
