...
import com.slilio.xiaohashu.count.biz.enums.CollectUnCollectNoteTypeEnum;
import com.slilio.xiaohashu.count.biz.model.dto.CountCollectUnCollectNoteMqDTO;
...
/**
 * @Author: slilio @CreateTime: 2025-05-13 @Description: 收藏计数 @Version: 1.0
 */
@Component
@RocketMQMessageListener(
    consumerGroup = "xiaohashu_group_" + MQConstants.TOPIC_COUNT_NOTE_COLLECT,
...
public class CountNoteCollectConsumer implements RocketMQListener<String> {
  ...
  private void consumeMessage(List<String> bodys) {
    ...
    Map<Long, List<CountCollectUnCollectNoteMqDTO>> groupMap =
        countCollectUnCollectNoteMqDTOS.stream()
    ...
    for (Map.Entry<Long, List<CountCollectUnCollectNoteMqDTO>> entry : groupMap.entrySet()) {
      ...
      for (CountCollectUnCollectNoteMqDTO countCollectUnCollectNoteMqDTO : list) {
        ...
        // 根据操作类型，获取对应枚举
        CollectUnCollectNoteTypeEnum collectUnCollectNoteTypeEnum =
            CollectUnCollectNoteTypeEnum.valueOf(type);
    ...
    countMap.forEach(
        (k, v) -> {
          ...
          if (isExisted) {
            ...
            redisTemplate
                .opsForHash()
                .increment(redisKey, RedisKeyConstants.FIELD_COLLECT_TOTAL, v);
          }
        });
    ...
    rocketMQTemplate.asyncSend(
        MQConstants.TOPIC_COUNT_NOTE_COLLECT_2_DB,
        message,
        new SendCallback() {
          @Override
