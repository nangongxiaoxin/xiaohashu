...
import com.slilio.xiaohashu.search.biz.service.UserService;
...
public class UserServiceImpl implements UserService {
  ...
  public PageResponse<SearchUserRspVO> searchUser(SearchUserReqVO searchUserReqVO) {
    ...
    // 构建SearchRequest,指定索引为USER （类似表名）
    SearchRequest searchRequest = new SearchRequest(UserIndex.NAME);

    // 构建查询内容（查询条件）
    SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
    // 构建multi_match查询，查询nickname、xiaohashu_id字段
    sourceBuilder.query(
    ...
    SortBuilder<?> sortBuilder =
        new FieldSortBuilder(UserIndex.FIELD_USER_FANS_TOTAL).order(SortOrder.DESC);
    // 排序
    ...
    highlightBuilder
        .field(UserIndex.FIELD_USER_NICKNAME)
    ...
    try {
      ...
      for (SearchHit hit : hits) {
        ...
        if (CollUtil.isNotEmpty(hit.getHighlightFields())
          ...
          highlightedNickname =
              hit.getHighlightFields().get(UserIndex.FIELD_USER_NICKNAME).fragments()[0].string();
        }

        // 构建VO实体类
        SearchUserRspVO searchUserRspVO =
            SearchUserRspVO.builder()
                .userId(userId)
                .nickname(nickname)
                .avatar(avatar)
