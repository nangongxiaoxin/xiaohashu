...
import com.slilio.xiaohashu.count.biz.enums.LikeUnlikeCommentTypeEnum;
import com.slilio.xiaohashu.count.biz.model.dto.AggregationCountLikeUnlikeCommentMqDTO;
import com.slilio.xiaohashu.count.biz.model.dto.CountLikeUnlikeCommentMqDTO;
...
@RocketMQMessageListener(
    consumerGroup = "xiaohashu_group_count_" + MQConstants.TOPIC_COMMENT_LIKE_OR_UNLIKE, // group组
    topic = MQConstants.TOPIC_COMMENT_LIKE_OR_UNLIKE // 主题topic
    )
@Slf4j
public class CountCommentLikeConsumer implements RocketMQListener<String> {
  ...
  private void consumeMessage(List<String> bodys) {
    ...
    List<CountLikeUnlikeCommentMqDTO> countLikeUnlikeCommentMqDTOS =
        bodys.stream()
            .map(body -> JsonUtils.parseObject(body, CountLikeUnlikeCommentMqDTO.class))
            .toList();

    // 按照评论ID进行分组
    Map<Long, List<CountLikeUnlikeCommentMqDTO>> groupMap =
        countLikeUnlikeCommentMqDTOS.stream()
    ...
    for (Map.Entry<Long, List<CountLikeUnlikeCommentMqDTO>> entry : groupMap.entrySet()) {
      ...
      for (CountLikeUnlikeCommentMqDTO countLikeUnlikeCommentMqDTO : list) {
        ...
        // 根据操作类型，获取对应枚举
        LikeUnlikeCommentTypeEnum likeUnlikeCommentTypeEnum =
            LikeUnlikeCommentTypeEnum.valueOf(type);
        // 若枚举为空
    ...
    countList.forEach(
        item -> {
          ...
          if (isCountCommentExisted) {
            // 对目标用户Hash中点赞数字字段进行计数操作
            redisTemplate
                .opsForHash()
                .increment(countCommentRedisKey, RedisKeyConstants.FIELD_LIKE_TOTAL, count);
          }
        });
    ...
    rocketMQTemplate.asyncSend(
        MQConstants.TOPIC_COUNT_COMMENT_LIKE_2_DB,
        message,
        new SendCallback() {
          @Override
